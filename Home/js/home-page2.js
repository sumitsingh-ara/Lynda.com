...
function showProducts2() {
  ...
  showPicks2.forEach(function (product, n) {
    ...
    function getData() {
      ...
      for (key in checker2) {
        courseKey2.push(checker2[key].name);
      }

      let inputs2 = {
        name2: product.name,
        price2: product.price,
        video2: product.video,
        date2: product.date,
        time2: product.time,
        by2: product.by,
        title2: product.title,
      };
      if (courseKey2.indexOf(product.name) != -1) {
        alert("This course is already added");
      } else {
        courseBag2.push(inputs2);
        localStorage.setItem("courseData", JSON.stringify(courseBag2));
