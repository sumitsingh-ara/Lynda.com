...
function showProducts3() {
  ...
  showPicks3.forEach(function (product, n) {
    ...
    function getData() {
      ...
      for (key in checker3) {
        courseKey3.push(checker3[key].name);
      }

      let inputs3 = {
        name3: product.name,
        price3: product.price,
        video3: product.video,
        date3: product.date,
        time3: product.time,
        by3: product.by,
        title3: product.title,
      };
      if (courseKey3.indexOf(product.name) != -1) {
        alert("This course is already added");
      } else {
        courseBag3.push(inputs3);
        localStorage.setItem("courseData", JSON.stringify(courseBag3));
